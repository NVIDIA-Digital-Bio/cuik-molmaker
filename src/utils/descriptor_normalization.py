...
DESCRIPTASTORUS_DESC_LIST = ['BalabanJ', 'BertzCT', 'Chi0', 'Chi0n', 'Chi0v', 'Chi1', 'Chi1n',
...
def get_continuous_cdf_fn(dist_name, params, data_stats):
    ...
    def cdf(v, dist=dist_func, arg=arg,loc=loc,scale=scale,minV=data_min,maxV=data_max):
        v = dist.cdf(np.clip(v, minV, maxV), loc=loc, scale=scale, *arg)
        return np.clip(v, 0., 1.)
    
    return cdf

def get_discrete_cdf_fn(params, data_stats):
    arr = np.array(params)
...
def get_normalization_functions(desc_list, param_dict):
    ...
    for desc_name in desc_list:
        ...
        if norm_type == "continuous":
            ...
            norm_func_dict[desc_name] = get_continuous_cdf_fn(dist_name, params, data_stats)
        elif norm_type == "discrete":
            params = desc_param_dict["params"]
            data_stats = desc_param_dict["data_stats"]
            norm_func_dict[desc_name] = get_discrete_cdf_fn(params, data_stats)
        else:
