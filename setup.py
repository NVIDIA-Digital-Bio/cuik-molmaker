...
# Copy data files
data_files = ['best_normalization_params.json', 'fast_normalization_params.json', 'descriptastorus_normalization_params.json']
...
setup(
    ...
    packages=find_packages(include=["cuik_molmaker", "cuik_molmaker.lib", "cuik_molmaker.utils", "cuik_molmaker.data"]),
    package_data={
        'cuik_molmaker': ['*.so', '*.py', 'data/*.json'],  # Include Python extension and Python files
        'cuik_molmaker.lib': ['*.so', '__init__.py'],  # Include shared libraries and __init__.py
...
so_file = os.path.join('build', f'cuik_molmaker_cpp.cpython-{PYTHON_DIGIT_ONLY_VERSION}-x86_64-linux-gnu.so')
